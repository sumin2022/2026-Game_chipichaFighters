...
void ServerMain::PostAccept() {
  ...
  BOOL ret = AcceptEx(server, accept_over->m_client_socket, accept_over->m_buff,
...
void ServerMain::HandleRecv(int player_index, DWORD num_bytes,
                            EXP_OVER *exp_over) {
  ...
  if (remain_data > 0) {
    memmove(session.m_recv_over.m_buff, packet_start, remain_data);
  }
...
void ServerMain::HandleAccept(EXP_OVER *exp_over) {
  ...
  CreateIoCompletionPort(reinterpret_cast<HANDLE>(client_socket), h_iocp,
                         new_id, 0);

...
bool RoomManager::join_room(int room_id, int player_id) {
  ...
  std::cout << "Player " << player_id << " joined room " << room_id << " ("
...
void RoomManager::select_character(int player_id, CharacterType character) {
  if (character < CHAR_DEALER || character > CHAR_HEALER)
    return;
...
void RoomManager::apply_character_to_player(PlayerState &state,
